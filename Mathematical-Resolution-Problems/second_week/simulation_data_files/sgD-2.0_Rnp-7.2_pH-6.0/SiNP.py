...
"""
Colloid fixed in the centre of the simulation box with surface groups and respective counterions. The surfacegroups have charge regulation. Repulsive LJ with offset for the interactions involving the colloid. Include visualization. 
"""
...
import espressomd
import sys
sys.path.insert(0,'/home/ritad/Desktop/np_pe_complexation/sugar_library/')
...
DESIRED_BLOCK_SIZE = 300 # desired number of samples per block
...
# consider multiplying by 1.3 to include the (30%) data that is ignored due to equilibration
 
#Calculate dependent parameters
##################### box size etc
R_COLL = np.sqrt(N_ACID / (4 * np.pi * SURFACE_CHARGE_DENS))  # Radius of colloid based on N_ACID and SURFACE_CHARGE_DENS
...
print("Tune skin")
system.cell_system.tune_skin(min_skin=0.1, max_skin =4.0, tol=0.1, int_steps=2000)
...
radii = {"Colloid": R_COLL_REDUCED, "Surf_A": PARTICLE_SIZE_REDUCED, "Surf_B": PARTICLE_SIZE_REDUCED, "Surf_HA": PARTICLE_SIZE_REDUCED, 
        "Na": PARTICLE_SIZE_REDUCED,  "Cl": PARTICLE_SIZE_REDUCED}
...
lj_eps = 5.0
lj_sig = 1.0
system.non_bonded_inter[types["Colloid"], types["Surf_B"]].lennard_jones.set_params(
               epsilon=lj_eps, sigma=lj_sig, cutoff=2**(1./6.)*lj_sig, offset =R_COLL_REDUCED, shift="auto")
system.non_bonded_inter[types["Colloid"], types["Na"]].lennard_jones.set_params(
               epsilon=lj_eps, sigma=lj_sig, cutoff=2**(1./6.)*lj_sig, offset =R_COLL_REDUCED, shift="auto")
system.non_bonded_inter[types["Colloid"], types["Cl"]].lennard_jones.set_params(
               epsilon=lj_eps, sigma=lj_sig, cutoff=2**(1./6.)*lj_sig, offset = R_COLL_REDUCED, shift="auto")
system.non_bonded_inter[types["Surf_B"], types["Surf_B"]].lennard_jones.set_params(
               epsilon=lj_eps, sigma=lj_sig, cutoff=2**(1./6.)*lj_sig, shift="auto")
system.non_bonded_inter[types["Surf_B"], types["Na"]].lennard_jones.set_params(
               epsilon=lj_eps, sigma=lj_sig, cutoff=2**(1./6.)*lj_sig, shift="auto")
system.non_bonded_inter[types["Surf_B"], types["Cl"]].lennard_jones.set_params(
               epsilon=lj_eps, sigma=lj_sig, cutoff=2**(1./6.)*lj_sig, shift="auto")
system.non_bonded_inter[types["Na"], types["Na"]].lennard_jones.set_params(
               epsilon=lj_eps, sigma=lj_sig, cutoff=2**(1./6.)*lj_sig, shift="auto")
system.non_bonded_inter[types["Na"], types["Cl"]].lennard_jones.set_params(
               epsilon=lj_eps, sigma=lj_sig, cutoff=2**(1./6.)*lj_sig, shift="auto")
system.non_bonded_inter[types["Cl"], types["Cl"]].lennard_jones.set_params(
               epsilon=lj_eps, sigma=lj_sig, cutoff=2**(1./6.)*lj_sig, shift="auto")
...
for sample in range(NUM_SAMPLES):
    ...
    if np.random.random() < PROB_INTEGRATION:
